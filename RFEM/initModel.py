...
try:
    ...
    if instSUDS == "y":
        ...
        try:
            ...
            print(
                'Please use command "pip install suds-py3 --user" in your Command Prompt.'
            )
            input("Press Enter to exit...")
            sys.exit()
    else:
...
try:
    ...
    if instSUDS == "y":
        ...
        try:
            ...
            print(
                'Please use command "pip install requests --user" in your Command Prompt.'
            )
            input("Press Enter to exit...")
            sys.exit()
    else:
...
try:
    ...
    if instSUDS == "y":
        ...
        try:
            ...
            print(
                'Please use command "pip install suds_requests --user" in your Command Prompt.'
            )
            input("Press Enter to exit...")
            sys.exit()
    else:
...
try:
    ...
    if instXML == "y":
        ...
        try:
            ...
            print(
                'Please use command "pip install xmltodict --user" in your Command Prompt.'
            )
            input("Press Enter to exit...")
            sys.exit()
    else:
...
def CheckIfMethodOrTypeExists(modelClient, method_or_type, unitTestMode=False):
    ...
    if method_or_type not in str(modelClient):
        if unitTestMode:
            ...
            assert False, (
                "WARNING: Used method/type: %s is not implemented in Web Services yet."
                % (method_or_type)
            )
    ...
    if currentStatus != status:
        ...
        if addOn in addonLst["__keylist__"]:
            ...
            for listType in addonLst["__keylist__"]:
                if (
                    not isinstance(addonLst[listType], bool)
                    and addOn in addonLst[listType]
                ):
...
def CalculateSelectedCases(
    loadCases: list = None, designSituations: list = None, loadCombinations: list = None
):
    """
    ...
    """
    specificObjectsToCalculate = Model.clientModel.factory.create(
    ...
    if designSituations is not None:
        for designSituation in designSituations:
            ...
            specificObjectsToCalculateDS.type = (
                ObjectTypes.E_OBJECT_TYPE_DESIGN_SITUATION.name
            )
...
def ExportResultTablesWithDetailedMembersResultsToCsv(TargetDirectoryPath: str):
    ...
    Model.clientModel.service.export_result_tables_with_detailed_members_results_to_csv(
        TargetDirectoryPath
    )
...
def ExportResultTablesWithDetailedMembersResultsToXML(TargetFilePath: str):
    ...
    Model.clientModel.service.export_result_tables_with_detailed_members_results_to_xml(
        TargetFilePath
    )
